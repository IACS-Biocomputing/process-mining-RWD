...
#' @param palette Optional, palette to use for coloring dots. Default is 'Set3'.
...
dotted_chart <- function(eventlog, x, sort, color, palette, units, ...) {
...
configure_x_labs <- function(x, units) {
	case_when(x == "relative" ~ as.character(glue("Time since episode start (in {units})")),
...
dotted_chart_plot <- function(data, mapping, x, y, color_vector, color_label, units) {
	...
	data %>%
		...
		labs(x = x_labs,y = "Episodes") +
		theme_light() -> p

	p + geom_point(aes(color = color)) + scale_color_manual(name = color_label, values = color_vector) -> p
...
dotted_chart.eventlog <- function(eventlog,
	...
	# For some reason the 'ifelse' only selects the first element of the palette
	if (is.null(palette))
	{
		color_vector = col_vector()
	}
	else
	{
	...
	eventlog %>%
		...
		dotted_chart_plot(mapping,
						  x,
						  y,
						  color_vector,
						  ifelse(is.null(color), activity_id(eventlog), color),
						  units)
}
